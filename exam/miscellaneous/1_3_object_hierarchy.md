...
**Containers** (e.g. databases) logically group objects (e.g. tables, views). **Hierarchy** refers to how these containers and objects are structured. Permissions can be set at the database, schema[...]

- **Organisation:** The highest level in Snowflake's hierarchy comprised of one or more Snowflake accounts
- **Account:** A single Snowflake deployment including all data, users, roles, and settings. Each account is deployed to a single cloud platform provider (AWS, Azure, GCP), to a single geographic r[...]
- **Database:** A logical object within an account to organize data. It can hold multiple schemas
- **Schema:** An object within a database that holds tables, views, stages, file formats, sequences, stored procedures, user-defined functions, and other objects. Schemas can be replicated to other[...]
